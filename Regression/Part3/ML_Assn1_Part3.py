...
def cost ( ThetaT, X, Y ):
    ...
    Power_Error = (Error).dot(Error.T)
    Cost = Factor*Power_Error[0][0]   
    return Cost 
...
os.makedirs("Hypothesis Functions/DatasetSize_Variation", exist_ok=True)
os.makedirs("Hypothesis Functions/CurveDegree_Variation", exist_ok=True)
os.makedirs("Cost vs Iteration", exist_ok=True)
os.makedirs("Cost vs Iteration/DatasetSize_Variation", exist_ok=True)
os.makedirs("Cost vs Iteration/CurveDegree_Variation", exist_ok=True)
os.makedirs("Error vs DatasetSize", exist_ok=True)
os.makedirs("Error vs Features", exist_ok=True)       
...
for ALPHA in LEARNING_RATES : 
    ...
    for M in DATASET_SIZE:
        os.makedirs("Hypothesis Functions/CurveDegree_Variation/DatasetSize_"+str(M), exist_ok=True)
        os.makedirs("Cost vs Iteration/CurveDegree_Variation/DatasetSize_"+str(M), exist_ok=True)
        ...
        for N in DEGREE :
            os.makedirs("Hypothesis Functions/DatasetSize_Variation/CurveDegree_"+str(N), exist_ok=True)
            os.makedirs("Cost vs Iteration/DatasetSize_Variation/CurveDegree_"+str(N), exist_ok=True)
            ...
            Training_Cost.append( MSE(ThetaT, Training_X, Train_Y) )
            Testing_Cost.append( MSE(ThetaT, Testing_X, Test_Y))
            ...
            plt.xlabel("X Values")
            plt.ylabel("Y Values")
            plt.title("Hypothesis Function of Degree" + str(N) + " (M=" + str(M) + ", Alpha=" + str(ALPHA) + ")" ) 
            plt.legend()
            plt.grid(True)
            os.chdir("Hypothesis Functions/DatasetSize_Variation/CurveDegree_"+str(N))
            plt.savefig( ("Degree" + str(N) + "Curve_M_" + str(M) + "_Alpha_" + str((int)(ALPHA*1000)) + 'm' ) )
            os.chdir("../../..")
            os.chdir("Hypothesis Functions/CurveDegree_Variation/DatasetSize_"+str(M))
            plt.savefig( ("Degree" + str(N) + "Curve_M_" + str(M) + "_Alpha_" + str((int)(ALPHA*1000)) + 'm' ) )
            os.chdir("../../..")
            #plt.show() 
            plt.close()
            ...
            plt.grid(True)
            os.chdir("Cost vs Iteration/DatasetSize_Variation/CurveDegree_"+str(N))            
            plt.savefig( ("Degree" + str(N) + "Curve_M_" + str(M) + "_Alpha_" + str((int)(ALPHA*1000)) + 'm'  ))
            os.chdir("../../..")
            os.chdir("Cost vs Iteration/CurveDegree_Variation/DatasetSize_"+str(M))            
            plt.savefig( ("Degree" + str(N) + "Curve_M_" + str(M) + "_Alpha_" + str((int)(ALPHA*1000)) + 'm'  ))
            os.chdir("../../..")
            #plt.show()
            plt.close()
        ...
        plt.title("VARIATION of Errors with Degree of Hypothesis Function (M=" + str(M) + ", Alpha=" + str(ALPHA) + ")" )
        plt.legend() 
        plt.grid(True)        
        plt.savefig( ("Errors_vs_Features_M_" + str(M) + "_Alpha_" + str((int)(ALPHA*1000))) + 'm' )
        #plt.show()
        plt.close()
        os.chdir("..") 
        ...
        print("")
        print(("PARAMETERS LEARNED for M="+str(M) + ", Alpha=" + str(ALPHA)).center(104) )
        print("-" * 104)
        print(DF)
        ...
        print("") 
        print( ("VARIATION OF ERRORS WITH FEATURES; M="+str(M) + ", Alpha=" + str(ALPHA)).center(51) ) 
        print("-" * 51)
        print(DF) 
    ...
    os.chdir("Error vs DatasetSize")
    for i in range(0, len(DEGREE)):
        plt.figure(figsize=(12, 8))
        plt.plot(DATASET_SIZE, Train_Costs[:, i], 'bo', label="Training Error")
        plt.plot(DATASET_SIZE, Test_Costs[:, i], 'go', label="Testing Error")
        plt.xscale("log")
        plt.xlabel("Size of Dataset")
        plt.ylabel("Mean Square Error")
        plt.title("VARIATION of Errors with Size of Dataset (N=" + str(DEGREE[i]) + ", Alpha=" + str(ALPHA) + ")" )
        plt.legend()
        plt.grid(True)        
        plt.savefig( ("Errors_vs_Size_N_" + str(DEGREE[i])  + "_Alpha_" + str((int)(ALPHA*1000))) + 'm' )
        #plt.show()
        plt.close()
    os.chdir("..")
